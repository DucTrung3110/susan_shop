...
class HomePage {
    ...
    async loadProductVariants() {
        try {
            ...
            this.products = this.products.map(product => {
                const productVariants = variants.filter(v => Number(v.product_id) === Number(product.id));
                console.log(`Product ${product.id} has ${productVariants.length} variants`);
                return {
                    ...product,
                    variants: productVariants
                };
            });
            console.log('Products with variants:', this.products);
        } catch (error) {
    ...
    renderProducts() {
        ...
        console.log('Sample product variants:', this.filteredProducts[0].variants);
        console.log('Sample product variants length:', this.filteredProducts[0].variants?.length);
        ...
        const html = this.filteredProducts.map(product => {
            ...
            console.log(`Product ${product.id}: min=${minPrice}, stock=${hasStock}`);
        ...
        grid.innerHTML = html;
        console.log('Grid children count after innerHTML:', grid.children.length);
